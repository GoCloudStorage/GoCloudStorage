...
func main() {
	...
	model.Init()
	storage.InitClient(opt.Cfg.Storage.Type, opt.Cfg.Storage.Endpoint, opt.Cfg.Storage.AccessKeyID, opt.Cfg.Storage.SecretAccessKey, opt.Cfg.Storage.BucketName, opt.Cfg.Storage.UseSSL)
	api.InitAPI(ctx)
	<-ctx.Done()
